                    ...
                    <div class="wrapper left2 left">
                        ...
                        <div class="body">
                            ...
                            <?php $i=1; $total=0; $submit=0; $adv=0; $rem=0; foreach($fee as $k=>$v){ ?>
                                <tr>
                                    <?php //var_dump($v['id']);die();?>
                                    <td class="td"><?php echo $i++; ?></td>
                                    <td class="td"><?php echo $v['id']; ?></td>
                                    <td class="td"><?php  $monthNum = sprintf("%02s",$date = $v['date']);
                                    //var_dump($monthNum);die();
                                    $monthName = date("F", strtotime($monthNum));echo $monthName;  ?></td>
                                    <td class="td">
                                    <?php 
                                    $otherfee=$this->voucher_model->other_fee($v['id']);
                                                // var_dump($otherfee);die();
                                                        $totalfee=$v['fee_pack'];
                                                        //var_dump($totalfee);die();
                                                        if($v['is_admitted']==1)
                                                        {
                                                            $totalfee+=$v['admin_fee'];
                                                        }
                                                        $date_submitted=date('Y-m-d',strtotime($v['submitted_at']));
                                                        $date_expiry=date('Y-m-d',strtotime($v['date_expire']));
                                                    if($this->voucher_model->other_fee($v['id'])!=false)
                                                    {
                                                    foreach($otherfee as $key=>$otherf)
                                                        {
                                                            $totalfee=$otherf['amount']+$totalfee;
                                                        }
                                                    }

                                                    if($date_submitted>$date_expiry) 
                                                    {
                                                        $fine=($v['fee_pack']*$v['late_fine'])/100;
                                                        $totalfee=$totalfee+$fine;
                                                    }
                                                     echo $totalfee;
                                                     $total=$total+$totalfee;
                                                     ?></td>
                                    <td class="td"><?php if($this->voucher_model->fee_history($monthNum,$v['student_id'])!=false) echo $this->voucher_model->fee_history($monthNum,$v['student_id']);else echo "0" ?></td>
                                    <td class="td"><?php echo $v['recieved'];  $submit=$submit+$v['recieved']; ?></td>
                                    <td class="td"><?php echo $v['remaining']; $rem=$v['remaining']; ?></td>
                                    <td class="td"><?php echo $v['advance']; $adv=$adv+$v['advance']; ?></td>
                                </tr>
                            ...
                            <div class="wd-100 clear">
                                <div class="wd-40 left">
                                    <div class="wd-50 left"><h3 class="text-left">Total Fee:</h3></div>
                                    <div class="wd-50 left"><h4><?php echo $total; ?></h4></div>
                                </div>

                                <div class="wd-50 left">
                                    <div class="wd-50 left"><h3 class="text-left">Submitted Fee:</h3></div>
                                    <div class="wd-50 left">
                                        <h4><?php echo  $submit; ?></h4>
                                    </div>
                                </div>
                            </div>
                            <div class="wd-100 clear">
                                <div class="wd-40 left">
                                    <div class="wd-50 left"><h3 class="text-left">Remainings:</h3></div>
                                    <div class="wd-50 left"><h4><?php echo  $rem; ?></h4></div>
                                </div>
                                <div class="wd-50 left">
                                    <div class="wd-50 left"><h3 class="text-left">Advance:</h3></div>
                                    <div class="wd-50 left"><h4><?php echo  $adv; ?></h4></div>
                                </div>
                            </div>
                            <div class="wd-100 clear">
