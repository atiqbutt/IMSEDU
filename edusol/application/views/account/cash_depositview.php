                           ...
                           <td style="width:100px;"><?php echo $value['headname'];   ?></td>
                            <td style="width:100px;"><?php echo $value['level2name'];   ?></td>
                             ...
                             <td style="width:100px;"><?php echo $value['b_name'];   ?></td>
                              <td style="width:100px;"><?php echo $value['submit_by'];   ?></td>
...
<script src="<?php echo base_url(); ?>assets/vendors/datatables.net/js/jquery.dataTables.min.js"></script>
    ...
    <script src="<?php echo base_url(); ?>assets/vendors/pdfmake/build/vfs_fonts.js"></script>
      ...
      $(document).ready(function(){
            ...
            $(document).on('click', '#print', function() {
              ...
              if(bank=='') {
                window.open("<?php echo base_url(); ?>Account/cash_depositprint/");
              }
               else if(bank!='' && program!='' && project!='' && main_head!='' && level2!='' && level3!='')
              {
                window.open("<?php echo base_url(); ?>Account/cash_depositprint/"+bank+"/"+program+"/"+project+"/"+main_head+"/"+level2+"/"+level3);
              }
              else if(bank!='' && program!='' && project!='' && main_head!='' && level2!='')
              {
                window.open("<?php echo base_url(); ?>Account/cash_depositprint/"+bank+"/"+program+"/"+project+"/"+main_head+"/"+level2);
              }
              else if(bank!='' && program!='' && project!='' && main_head!='' )
              {
               window.open("<?php echo base_url(); ?>Account/cash_depositprint/"+bank+"/"+program+"/"+project+"/"+main_head);
              }
              else if(bank!='' && program!='' && project!='' ) {
                window.open("<?php echo base_url(); ?>Account/cash_depositprint/"+bank+"/"+program+"/"+project);
              }
              else if(bank!='' && program!='' ) {
                window.open("<?php echo base_url(); ?>Account/cash_depositprint/"+bank+"/"+program);
              }
              else if(bank!='') {
                window.open("<?php echo base_url(); ?>Account/cash_depositprint/"+bank);
              }
            });
        ...
        $('#level2').change(function(){
         ...
         $('#program').change(function(){
          ...
          $.ajax({
                    url: "<?php echo base_url(); ?>Load_account/programe_against_cash_deposit/"+id,
                    data: {},
            ...
            $.each(d, function(i,v){
                       ...
                       rt += "<tr><td>"+(++i)+"</td><td>"+v.programename+"</td><td>"+v.p_name+"</td><td>"+v.headname+"</td><td>"+v.level2name+"</td><td>"+v.level3name+"</td><td>"+v.b_name+"</td><td>"+v.submit_by+"</td><td>"+v.description+"</td><td>"+v.amount+"</td><td>"+v.date+"</td></tr>";
                     });
         ...
         $('#bank').change(function(){
          ...
          $.ajax({
                    url: "<?php echo base_url(); ?>Load_account/bank_against_cash_deposit/"+id,
                    data: {},
                      success: function( ret ) {
                          var d=jQuery.parseJSON(ret);
    ...
    $('#project').change(function(){
          ...
          $.ajax({
                    url: "<?php echo base_url(); ?>Load_account/project_against_cash_deposit/"+id,
                    data: {},
                      success: function( ret ) {
                          var d=jQuery.parseJSON(ret);
         ...
         $('#main').change(function(){
          ...
          var p=$('#project').val();
          $.ajax({
                    url: "<?php echo base_url(); ?>Load_account/main_head_against_cash_deposit/"+id+"/"+p,
                    data: {},
                       success: function( ret ) {
                          var d=jQuery.parseJSON(ret);
...
$('#level2').change(function(){
          ...
          var m=$('#main').val();
          $.ajax({
                    url: "<?php echo base_url(); ?>Load_account/level2_against_cash_deposit/"+m+"/"+id+"/"+p,
                    data: {},
                      success: function( ret ) {
                          var d=jQuery.parseJSON(ret);
...
$('#level3').change(function(){
          ...
          var l2=$('#level2').val();
          $.ajax({
                    url: "<?php echo base_url(); ?>Load_account/level3_against_cash_deposit/"+m+"/"+id+"/"+p+"/"+l2,
                    data: {},
                      success: function( ret ) {
                          var d=jQuery.parseJSON(ret);
