...
class Voucher extends CI_Controller {
	...
	{
		$students = $this->db->select("id,student_id")->from("invoice")->get()->result();	
		foreach($students as $k=>$v)
		{
			$prom_id = $this->db->select("id")->from("promotion")->where("is_active","1")->where("is_delete","0")->where("student_id",$v->student_id)->get()->row()->id;
			//echo ($v->student_id." -- ".$prom_id."<br>");
			$data = array(
				"student_id"=>$prom_id
			);
			$this->db->where("id",$v->id)->update("invoice",$data);
		}
	}
